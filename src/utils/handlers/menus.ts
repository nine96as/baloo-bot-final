...
export default () : Collection<string, SelectMenu> => {
    ...
    const files = getFiles(join(__dirname, '../../interactions/components/menus'))

    files.forEach((filePath) => {
        const menu: SelectMenu = require(filePath).default;
        if (menu === undefined || menu.data === undefined) {
            logger.error(`file at path ${filePath} seems to be incorrectly be exporting a select menu.`)
        } else {
