...
class Clear extends Command {
    ...
    public async execute(interaction: ChatInputCommandInteraction, client: Bot) {
        if (interaction.inCachedGuild()) {
            ...
            if (amount! > 100) {
                interaction.reply('❌ | you cannot clear more than 100 messages at once.')
            } else {
                if (target) {
                    let i = 0;
                    ...
                    try {
                        ...
                        logger.error(e);
                        interaction.reply('❌ | there was an error while attempting to delete the messages.');
                    }
                } else {
                    try {
                        await channel?.bulkDelete(amount!, true);
                        interaction.reply(`🚨 | cleared ${amount} message(s) from this channel.`);
                    } catch (e) {
                        logger.error(e);
                        interaction.reply('❌ | there was an error while attempting to delete the messages.');
                    }
